...
public:
    ...
    /*
	 ...
	 La operación es lineal, porque recorre la lista hasta _numElems en el peor de los casos.
	...
	*/
    void estremece() {
		...
		if (_numElems > 2 ) {
			Nodo *aux = _prim->_sig;
			Nodo *orgUlt = _ult;
			unsigned int i = 2; //Primera posicion la 0.
			//Principio;
			_prim->_sig = aux->_sig;
			Nodo *sprim = _prim->_sig;
			sprim->_ant = _prim;
			...
			while (i < _numElems - 1) {
				if (i % 2 == 0) { //Par
					...
					//Principio
					Nodo *sigAux = aux->_sig;
					sprim->_sig = aux->_sig;
					sigAux->_ant = sprim;
					sprim = sigAux;

					//Cola
					Nodo *dOrgUlt = orgUlt->_sig;
					aux->_sig = orgUlt->_sig;
					dOrgUlt->_ant = aux;
					orgUlt->_sig = aux;
					aux->_ant = orgUlt;
